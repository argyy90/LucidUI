...
local function FormatMoney(copper)
  ...
  if gold > 0 then
    return string.format("|cffffd700%s|r%s |cffc7c7cf%d|r%s |cffeda55f%d|r%s", FormatGoldNumber(gold), GOLD_ICON, silver, SILVER_ICON, cop, COPPER_ICON)
...
local function CreateSlot(parent, index)
  ...
  -- Strip ALL default template textures for a clean slate
  ...
  -- 1px black border INSIDE the slot bg edge
  ...
  -- Quality border (colored, drawn on top of slot border, INSIDE bg bounds)
  ...
  -- Hover highlight (anchored to bg, not full slot)
  ...
  -- Keep default ContainerFrameItemButtonTemplate OnClick behavior
  -- (handles UseContainerItem, PickupContainerItem, split stack, etc. natively)

  -- Clear new item status on mouseover
  ...
  -- Cache icon reference and crop edges
  ...
  -- Upgrade arrow (green arrow on items that are an upgrade)
  ...
  -- Bind type text (BOE/BOU) — top center
  ...
  -- New item glow (gold pulsing border)
  local newBorders = {}
  ...
  -- Pulse animation group for new item
...
local function ApplySlotAnchors(slot)
  ...
  -- Item level position + size (anchored to _bg, not slot frame)
  ...
  -- Count position + size (anchored to _bg)
  if slot.Count then
  ...
  -- Slot background alpha (transparent mode makes slots see-through)
...
local function UpdateSlot(slot)
  ...
  if info.iconFileID then
    ...
    -- Quality border (thicker for epic+, thinner for uncommon/rare)
    ...
    -- Shared ItemLocation for ilvl + bind checks
    ...
    -- Item level (only on actual gear: weapons + armor)
    ...
    -- Junk icon + desaturation
    local isJunk = info.quality == 0 and not info.noValue
    if DB("bagShowJunk") and isJunk then
      slot._junk:Show()
    else
      slot._junk:Hide()
    end
    ...
    if itemExists and info.hyperlink then
      ...
      if not isBound then
        ...
        if bindType == BIND_EQUIP then
          -- Check if it's a Warband item (bound to account until equipped)
          local isWuE = C_Item.IsBoundToAccountUntilEquip and C_Item.IsBoundToAccountUntilEquip(itemLoc)
          if isWuE then
            slot._bindText:SetText("WuE")
            slot._bindText:SetTextColor(0.0, 0.8, 1)
          else
            slot._bindText:SetText("BOE")
            slot._bindText:SetTextColor(0.1, 1, 0.1)
          end
          slot._bindText:Show(); showBind = true
        elseif bindType == BIND_USE then
          slot._bindText:SetText("BOU")
          slot._bindText:SetTextColor(1, 0.8, 0.2)
          slot._bindText:Show(); showBind = true
        end
      end
    end
    ...
    -- New item: gold pulsing border
    if DB("bagNewItemGlow") and C_NewItems and C_NewItems.IsNewItem(bagID, slotID) then
...
local function LayoutBags()
  ...
  if splitBags then
    ...
    for _, bagID in ipairs(bagOrder) do
      ...
      for slotID = 1, numSlots do
        group[#group + 1] = {bagID = bagID, slotID = slotID}
      end
      if #group > 0 then groups[#groups + 1] = group end
    end
  else
    -- All regular bags combined
    local group = {}
  ...
  -- Reagent bag: if split, render in own window; otherwise add as separate group (with splitSpacing gap)
  local reagentGroup = {}
  local rSlots = C_Container.GetContainerNumSlots(REAGENT_BAG)
  if rSlots and rSlots > 0 then
    if splitReagent then
      -- Will be rendered in separate window
      for slotID = 1, rSlots do
        reagentGroup[#reagentGroup + 1] = {bagID = REAGENT_BAG, slotID = slotID}
      end
    else
      -- Add as own group so splitSpacing creates a gap before it
      local rGroup = {}
      for slotID = 1, rSlots do
        rGroup[#rGroup + 1] = {bagID = REAGENT_BAG, slotID = slotID}
      end
      if #rGroup > 0 then groups[#groups + 1] = rGroup end
    end
  end

  -- Reverse each group if desired
  ...
  -- Layout all groups with spacing between them
  local contentFrame = bagFrame._content
  local slotIdx = 0
  local yOffset = 0

  for g, group in ipairs(groups) do
    -- Add separator spacing between groups
    if g > 1 then
      yOffset = yOffset + splitSpacing
    end
    ...
    for i, slotData in ipairs(group) do
      ...
      if not slot then
        slot = CreateSlot(contentFrame, slotIdx)
        slots[slotIdx] = slot
      end
      ...
      -- Set slot frame to fill the grid cell so there are no dead zones for item drops
      local cellSize = iconSize + spacing
      slot:SetSize(cellSize, cellSize)
      -- Constrain the icon within the cell
      local icon = slot._iconRef
      if icon then
        icon:ClearAllPoints()
      ...
      -- Adjust bg to icon area (borders follow automatically since anchored to bg)
      slot._bg:ClearAllPoints()
      slot._bg:SetPoint("TOPLEFT", 0, 0)
      slot._bg:SetSize(iconSize, iconSize)

      local localIdx = i - 1
      local col = localIdx % columns
      local row = math.floor(localIdx / columns)
      slot:ClearAllPoints()
      slot:SetPoint("TOPLEFT", contentFrame, "TOPLEFT",
        col * cellSize,
      ...
      -- Slot bg color — tint reagent slots when merged
    ...
    -- Track reagent section position for inline accent border
  ...
  -- Inline reagent accent border (when not split into own window)
  ...
  if not splitReagent and bagFrame._reagentInlineY then
    ...
    rib:Hide()
    bagFrame._reagentInlineY = nil
  end

  -- ── Reagent bag separate window ──────────────────────────
  if not bagFrame._reagentWin then
    ...
    rtitle:SetTextColor(CYAN[1], CYAN[2], CYAN[3])
    rtitle:SetText("Reagents")
    rw._title = rtitle
    -- Content
  ...
  if splitReagent and #reagentGroup > 0 then
    ...
    -- Compact layout: 5 columns, anchored left of bags
    local rCols = 5
    ...
    for i, slotData in ipairs(reagentGroup) do
      ...
      if not slot then
        slot = CreateSlot(rContent, 1000 + i)
        bagFrame._reagentSlots[i] = slot
      end
      slot._bagID = slotData.bagID
      slot._slotID = slotData.slotID
      slot:SetID(slotData.slotID)
      ...
      local col = (i - 1) % rCols
      local row = math.floor((i - 1) / rCols)
      slot:ClearAllPoints()
      slot:SetPoint("TOPLEFT", rContent, "TOPLEFT", col * cellSize, -row * cellSize)
      slot:Show()
      ApplySlotAnchors(slot)
      UpdateSlot(slot)
    end
    ...
    -- Size and position: anchored to bag frame bottom-left, grows upward
    local rTitleH = 18
    local pad = 4
    ...
    rWin:ClearAllPoints()
    rWin:SetPoint("BOTTOMRIGHT", bagFrame, "BOTTOMLEFT", -2, 0)
    -- Update accent color
  ...
  -- Update currencies (tracked backpack tokens, multi-line)
  if bagFrame._currencyContainer then
    -- Gather all tracked currencies
    local entries = {}
    ...
    -- Split into lines that fit within frame width
    ...
    for _, entry in ipairs(entries) do
      -- Test if adding this entry exceeds width
    ...
    -- Create/update FontStrings per line
    local lineH = 16
    ...
    -- Hide unused
    for li = #lines + 1, #bagFrame._currencyLines do
      bagFrame._currencyLines[li]:Hide()
    end
    -- Resize currency container
    local numLines = math.max(#lines, 1)
    bagFrame._currencyBg:SetHeight(numLines * lineH)
  end

  -- Resize frame (after currency so height is correct)
...
local function BuildBagFrame()
  ...
  bagFrame:SetResizeBounds(200, 150)
  ...
  if pos then
    ...
    bagFrame:SetPoint(pos[1], UIParent, pos[2], pos[3], pos[4])
  end
  ...
  closeTex:SetTexture("Interface/AddOns/LucidUI/Assets/X_red.png")
  ...
  -- Settings cog (opens LucidUI settings on Bags tab)
  local cogBtn = CreateFrame("Button", nil, bagFrame)
  cogBtn:SetSize(16, 16)
  cogBtn:SetPoint("RIGHT", closeBtn, "LEFT", -4, 0)
  local cogTex = cogBtn:CreateTexture(nil, "ARTWORK")
  cogTex:SetAllPoints()
  cogTex:SetTexture("Interface/AddOns/LucidUI/Assets/Cog.png")
  cogTex:SetVertexColor(0.6, 0.6, 0.6)
  cogBtn:SetScript("OnClick", function()
    -- Open LucidUI settings and select Bags tab
  ...
  cogBtn:SetScript("OnEnter", function()
    ...
    GameTooltip:SetOwner(cogBtn, "ANCHOR_LEFT")
    GameTooltip:SetText("Bags Settings", 1, 1, 1)
    GameTooltip:Show()
  end)
  ...
  -- Sort button (WoW's cleanup icon)
  local sortBtn = CreateFrame("Button", nil, bagFrame)
  sortBtn:SetSize(16, 16)
  sortBtn:SetPoint("RIGHT", cogBtn, "LEFT", -4, 0)
  ...
  sortTex:SetVertexColor(0.6, 0.6, 0.6)
  ...
  -- Bags toggle button (backpack icon, toggles bag bar)
  local bagsBtn = CreateFrame("Button", nil, bagFrame)
  bagsBtn:SetSize(16, 16)
  bagsBtn:SetPoint("RIGHT", sortBtn, "LEFT", -6, 0)
  local bagsBtnTex = bagsBtn:CreateTexture(nil, "ARTWORK")
  bagsBtnTex:SetAllPoints()
  bagsBtnTex:SetTexture(130716) -- Backpack icon
  bagsBtnTex:SetTexCoord(0.08, 0.92, 0.08, 0.92)
  bagsBtnTex:SetVertexColor(0.6, 0.6, 0.6)

  -- Bag bar container (appears above the frame, outside)
  local bagBar = CreateFrame("Frame", nil, bagFrame, "BackdropTemplate")
  local BAG_BAR_SIZE = 28
  local BAG_BAR_SPACING = 4
  local numBagBtns = 1 + NUM_BAG_SLOTS + 1 -- backpack + bags + reagent
  ...
  bagBar:SetFrameStrata("MEDIUM")
  bagBar:Hide()
  bagFrame._bagBar = bagBar

  -- Accent line on bag bar
  ...
  -- Backpack button in bag bar
  local bpBtn = CreateFrame("Button", nil, bagBar)
  bpBtn:SetSize(BAG_BAR_SIZE, BAG_BAR_SIZE)
  bpBtn:SetPoint("LEFT", bagBar, "LEFT", 6, 0)
  local bpTex = bpBtn:CreateTexture(nil, "ARTWORK")
  bpTex:SetAllPoints()
  bpTex:SetTexture(130716)
  bpTex:SetTexCoord(0.08, 0.92, 0.08, 0.92)
  ...
  bpBtn:SetScript("OnEnter", function()
    GameTooltip:SetOwner(bpBtn, "ANCHOR_TOP")
  ...
  if rNumSlots and rNumSlots > 0 then
    ...
    local rTex = rBtn:CreateTexture(nil, "ARTWORK")
    rTex:SetAllPoints()
    ...
    rBtn:SetScript("OnEnter", function()
      GameTooltip:SetOwner(rBtn, "ANCHOR_TOP")
      GameTooltip:SetInventoryItem("player", rInvID)
      GameTooltip:Show()
    end)
    rBtn:SetScript("OnLeave", function() GameTooltip:Hide() end)
  end

  -- Toggle bag bar on click
  bagsBtn:SetScript("OnClick", function()
    bagBar:SetShown(not bagBar:IsShown())
  end)
  bagsBtn:SetScript("OnEnter", function()
    bagsBtnTex:SetVertexColor(1, 1, 1)
    GameTooltip:SetOwner(bagsBtn, "ANCHOR_LEFT")
    GameTooltip:SetText("Toggle Bag Bar", 1, 1, 1)
    GameTooltip:Show()
  end)
  bagsBtn:SetScript("OnLeave", function()
    bagsBtnTex:SetVertexColor(0.6, 0.6, 0.6)
    GameTooltip:Hide()
  end)
  ...
  -- Dark search bar background
  local searchBg = searchBox:CreateTexture(nil, "BACKGROUND")
  searchBg:SetAllPoints()
  searchBg:SetColorTexture(0.05, 0.05, 0.05, 1)
  -- Hide default search box textures
  ...
  -- Gold display in title bar (next to "Bags")
  ...
  -- Currency display at bottom (no background, just text)
  local currencyBg = CreateFrame("Frame", nil, bagFrame)
  currencyBg:SetPoint("BOTTOMLEFT", 8, 4)
  currencyBg:SetPoint("BOTTOMRIGHT", -8, 4)
  currencyBg:SetHeight(16)
  ...
  -- Widen BackpackTokenFrame so WoW allows tracking more than 1 currency
  ...
  -- Hook TokenFrame.SetTokenWatched for live currency updates when player changes tracked currencies
  ...
  bagFrame:SetScript("OnEvent", function(self, event, ...)
    -- Currency/money events always process for live updates
    if event == "PLAYER_MONEY" or event == "CURRENCY_DISPLAY_UPDATE" or event == "PLAYER_ENTERING_WORLD" then
      if self:IsShown() then
        self._moneyText:SetText(FormatMoney(GetMoney()))
        LayoutBags()
      end
      return
    end
    ...
    elseif event == "BAG_UPDATE" then
      -- Quick update for the changed bag
      local bagID = ...
      for _, slot in ipairs(slots) do
        if slot:IsShown() and slot._bagID == bagID then
          UpdateSlot(slot)
        end
      end
...
function B.OpenBags()
  ...
  if not bagFrame:IsShown() then
    bagFrame:Show()
    LayoutBags()
    -- Delayed refresh for currencies that may not be loaded yet
...
RefreshBagBarIcons = function()
  ...
  -- Also update reagent bag button if exists
...
function B.RefreshLayout()
  ...
  -- Throttle: only refresh once per 0.05s to avoid slider lag
...
hookFrame:SetScript("OnEvent", function(self)
  ...
  -- Auto-open bags at bank, mail, AH
...
function B.SetupSettings(parent)
  ...
  -- Helper: create a row with two checkboxes side by side
  ...
  iconSize.option = "bagIconSize"
  table.insert(allFrames, iconSize)

  local spacingSlider = NS.ChatGetSlider(scrollChild, "Spacing", 0, 12, "%dpx", function(v) DBSet("bagSpacing", v); B.RefreshLayout() end)
  spacingSlider.option = "bagSpacing"
  table.insert(allFrames, spacingSlider)

  local columnsSlider = NS.ChatGetSlider(scrollChild, "Columns", 4, 20, "%d", function(v) DBSet("bagColumns", v); B.RefreshLayout() end)
  columnsSlider.option = "bagColumns"
  table.insert(allFrames, columnsSlider)

  -- ── Display (paired checkboxes) ──────────────────────────
  local displayHeader = NS.ChatGetHeader(scrollChild, "Display")
  table.insert(allFrames, displayHeader)

  local showIlvl = NS.ChatGetCheckbox(scrollChild, "Item Level", 28, function(s) DBSet("bagShowIlvl", s); B.RefreshLayout() end, "Show item level on gear")
  showIlvl.option = "bagShowIlvl"
  local showCount = NS.ChatGetCheckbox(scrollChild, "Stack Count", 28, function(s) DBSet("bagShowCount", s); B.RefreshLayout() end, "Show stack count")
  showCount.option = "bagShowCount"
  table.insert(allFrames, MakePairRow(showIlvl, showCount))

  local showQuality = NS.ChatGetCheckbox(scrollChild, "Quality Borders", 28, function(s) DBSet("bagShowQuality", s); B.RefreshLayout() end, "Colored borders by quality")
  showQuality.option = "bagShowQuality"
  local showJunk = NS.ChatGetCheckbox(scrollChild, "Junk Icon", 28, function(s) DBSet("bagShowJunk", s); B.RefreshLayout() end, "Coin icon on junk items")
  showJunk.option = "bagShowJunk"
  table.insert(allFrames, MakePairRow(showQuality, showJunk))

  local junkDesat = NS.ChatGetCheckbox(scrollChild, "Desaturate Junk", 28, function(s) DBSet("bagJunkDesaturate", s); B.RefreshLayout() end, "Grey out junk items")
  junkDesat.option = "bagJunkDesaturate"
  local newGlow = NS.ChatGetCheckbox(scrollChild, "New Item Glow", 28, function(s) DBSet("bagNewItemGlow", s); B.RefreshLayout() end, "Gold pulsing border on new items")
  newGlow.option = "bagNewItemGlow"
  table.insert(allFrames, MakePairRow(junkDesat, newGlow))

  local questIcon = NS.ChatGetCheckbox(scrollChild, "Quest Icon", 28, function(s) DBSet("bagQuestIcon", s); B.RefreshLayout() end, "Quest icon on quest items")
  questIcon.option = "bagQuestIcon"
  local upgradeArrow = NS.ChatGetCheckbox(scrollChild, "Upgrade Arrow", 28, function(s) DBSet("bagShowUpgrade", s); B.RefreshLayout() end, "Green arrow on items that are an upgrade")
  upgradeArrow.option = "bagShowUpgrade"
  table.insert(allFrames, MakePairRow(questIcon, upgradeArrow))

  local reverseSlots = NS.ChatGetCheckbox(scrollChild, "Reverse Slot Order", 28, function(s) DBSet("bagSortReverse", s); B.RefreshLayout() end, "Reverse item order")
  reverseSlots.option = "bagSortReverse"
  table.insert(allFrames, reverseSlots)

  -- ── Bag Splitting (paired) ──────────────────────────
  local splitHeader = NS.ChatGetHeader(scrollChild, "Splitting")
  table.insert(allFrames, splitHeader)

  local splitReagent = NS.ChatGetCheckbox(scrollChild, "Separate Reagent Bag", 28, function(s) DBSet("bagSplitReagent", s); B.RefreshLayout() end, "Reagent bag in own window")
  splitReagent.option = "bagSplitReagent"
  local splitBags = NS.ChatGetCheckbox(scrollChild, "Split Individual Bags", 28, function(s) DBSet("bagSplitBags", s); B.RefreshLayout() end, "Gap between each bag")
  splitBags.option = "bagSplitBags"
  ...
  splitSpacing.option = "bagSplitSpacing"
  table.insert(allFrames, splitSpacing)

  -- ── Auto Open (paired) ──────────────────────────
  local autoHeader = NS.ChatGetHeader(scrollChild, "Auto Open")
  table.insert(allFrames, autoHeader)

  local autoBank = NS.ChatGetCheckbox(scrollChild, "Bank", 28, function(s) DBSet("bagAutoBank", s) end, "Auto-open at bank")
  autoBank.option = "bagAutoBank"
  local autoMail = NS.ChatGetCheckbox(scrollChild, "Mailbox", 28, function(s) DBSet("bagAutoMail", s) end, "Auto-open at mailbox")
  autoMail.option = "bagAutoMail"
  table.insert(allFrames, MakePairRow(autoBank, autoMail))

  local autoAH = NS.ChatGetCheckbox(scrollChild, "Auction House", 28, function(s) DBSet("bagAutoAH", s) end, "Auto-open at AH")
  autoAH.option = "bagAutoAH"
  local transparentCB = NS.ChatGetCheckbox(scrollChild, "Transparent Slots", 28, function(s) DBSet("bagTransparent", s); B.RefreshLayout() end, "Semi-transparent slot backgrounds")
  transparentCB.option = "bagTransparent"
  ...
  ilvlSizeSlider.option = "bagIlvlSize"
  table.insert(allFrames, ilvlSizeSlider)
  ...
  countSizeSlider.option = "bagCountSize"
  table.insert(allFrames, countSizeSlider)
  ...
  local slotAlpha = NS.ChatGetSlider(scrollChild, "Slot Opacity", 0, 100, "%d%%", function(value)
    ...
    for _, slot in ipairs(slots) do
      if slot:IsShown() and slot._bg then
        ...
        slot._bg:SetColorTexture(r, g, b, alpha)
        slot._bgAlpha = alpha
      end
    end
  end)
  slotAlpha.option = "bagSlotBgAlpha"
  slotAlpha._isPercent = true
  ...
  container:SetScript("OnShow", function()
    for _, f in ipairs(allFrames) do
      if f.SetValue and f.option then
        local val = DB(f.option)
        if val ~= nil then f:SetValue(val) end
      end
      -- Update paired checkboxes inside rows
      if f._cb1 and f._cb1.SetValue and f._cb1.option then
        local v = DB(f._cb1.option); if v ~= nil then f._cb1:SetValue(v) end
      end
      if f._cb2 and f._cb2.SetValue and f._cb2.option then
        local v = DB(f._cb2.option); if v ~= nil then f._cb2:SetValue(v) end
      end
    end
  end)
...
-- ── Suppress UseContainerItem taint (harmless, affects all bag addons) ──
